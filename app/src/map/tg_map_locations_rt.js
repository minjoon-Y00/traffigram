	...
	calTargetAndRealNodes() {
  	...
  	const cLng = this.map.tgOrigin.origin.original.lng;
		...
		for(let loc of this.locations[this.currentType]) {
			...
			const targetLen = this.map.calLenFromLatLng(targetPos.lat, targetPos.lng);
			//const targetLen = tgUtil.D2(cLat, cLng, targetPos.lat, targetPos.lng);
			//const targetLen = tgUtil.distance(cLat, cLng, targetPos.lat, targetPos.lng);
			...
			loc.node.target.lng = 
					cLng + targetLen * Math.sin(realDegree) * this.graph.magnify();
					//cLng + targetLen * Math.sin(realDegree) * this.graph.toLat();
		}
